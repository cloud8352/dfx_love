...
local path = string.sub(..., 1, string.len(...) - string.len(".skins.dnf.skin"))
...
function skin.DrawFrame(object)
  ...
  local topbarimage2 = skin.images["frame-topbar2.png"]
  ...
  --love.graphics.draw(topbarimage2, x, y+5, 0, topbarimage_scalex, topbarimage_scaley)
...
function skin.DrawButton(object)
  ...
  local scalex = width/image_hover:getWidth()
	local scaley = height/image_hover:getHeight()
	
	if not enabled or not clickable then
		-- button body
		love.graphics.setColor(255, 255, 255, 255)
		love.graphics.draw(skin.images["button-unclickable.png"], x, y, 0, scalex, scaley)
		-- button text
		love.graphics.setFont(font)
	...
	if object.toggleable then
		if hover then
			if down then
				-- button body
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(skin.images["button-down.png"], x, y, 0, scalex, scaley)
				-- button text
				love.graphics.setFont(font)
				...
				-- button body
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(image_hover, x, y, 0, scalex, scaley)
				-- button text
				love.graphics.setFont(font)
			...
			if object.toggle then
				...
				-- button body
				love.graphics.setColor(255, 255, 255, 255)
				love.graphics.draw(skin.images["button-nohover.png"], x, y, 0, scalex, scaley)
				-- button text
				love.graphics.setFont(font)
...
function skin.DrawImage(object)
  ...
  local image_nil = skin.images["nil.png"]
  local draw_width = object.draw_width  --图片显示尺寸
  local draw_height = object.draw_height
  
  if image then
  	scalex = draw_width/image:getWidth()
    scaley = draw_height/image:getHeight()
    ...
    love.graphics.setColor(255, 255, 255, 255)
    love.graphics.draw(image_nil, x, y, orientation, scalex, scaley, offsetx, offsety, shearx, sheary)
  end
...
function skin.DrawImageButton(object)
  ...
  local image_hover = object.image_hover  --鼠标接触时显示的图片
  local image_down = object.image_down --按钮按下时显示的图片
	local image_scalex = draw_width/image:getWidth()
	local image_scaley = draw_height/image:getHeight() 
  local image_hover_scalex = draw_width/image:getWidth()
	local image_hover_scaley = draw_height/image:getHeight() 
	local image_down_scalex = draw_width/image:getWidth()
	local image_down_scaley = draw_height/image:getHeight() 
  if image_hover then
    image_hover_scalex = draw_width/image_hover:getWidth()
    image_hover_scaley = draw_height/image_hover:getHeight()
  end
  if image_down then
    image_down_scalex = draw_width/image_down:getWidth()
    image_down_scaley = draw_height/image_down:getHeight()
  end
	...
	if down then
      ...
      love.graphics.setColor(imagecolor)
			love.graphics.draw(image_down, x, y, 0, image_down_scalex, image_down_scaley)
    elseif image then
			love.graphics.setColor(imagecolor)
			love.graphics.draw(image, x + 1, y + 1, 0, image_scalex, image_scaley)
		end
		love.graphics.setFont(font)
	...
	elseif hover then
		if image_hover then
      love.graphics.setColor(imagecolor)
			love.graphics.draw(image_hover, x, y, 0, image_hover_scalex, image_hover_scaley)
    elseif image then
			love.graphics.setColor(imagecolor)
			love.graphics.draw(image, x, y, 0, image_scalex, image_scaley)
		end
		love.graphics.setFont(font)
		love.graphics.setColor(0, 0, 0, 255)
